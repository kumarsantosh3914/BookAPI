...
class BookService {
  ...
  updateRating = async (userId, bookId, rating) => {
    try {
      ...
      const totalBookRatingCount =
        await this.bookRepository.getTotalBookRatings(bookId);
      const userBookRating = await this.bookRepository.getBookRatingByAUser(
        bookId,
        userId
      );
      ...
      logger.error(
        "Something went wrong in the Book Service : updateRating",
        error
      );
      throw error;
    }
